  ...
  dragger(draggerIn)
{
  ...
  //// 11.28 新增：尝试转换成 ViewProviderDragger
  //m_vpDragger = dynamic_cast<ViewProviderDragger*>(vpObjectIn);
...
void TaskCSysDragger::setupGui()
{
  ...
  //// 11.28 新增：
  //auto dLabel = new QLabel(tr("Drag distance:"), incrementsBox);
  //gridLayout->addWidget(dLabel, 2, 0, Qt::AlignRight);

  //distanceSpinBox = new QuantitySpinBox(incrementsBox);
  //distanceSpinBox->setMinimum(0.0);
  //distanceSpinBox->setMaximum(std::numeric_limits<double>::max());
  //distanceSpinBox->setUnit(Base::Unit::Length); 
  //distanceSpinBox->setMinimumWidth(spinBoxWidth);
  //gridLayout->addWidget(distanceSpinBox, 2, 1, Qt::AlignLeft);
  ...
  //// 11.28 新增：
  //connect(distanceSpinBox, &QAbstractSpinBox::editingFinished, this, &TaskCSysDragger::onDistanceEditingFinished);
...
//   dragger->rotationIncrement.setValue(lastRotationIncrement);
  ...
  //// 11.28 新增：初始化拖动距离（如果有）
  //if (m_vpDragger && distanceSpinBox) {
  //    double d = m_vpDragger->getCurrentDragDistance();
  //    distanceSpinBox->setValue(d > 0.0 ? d : 0.0);
  //}
...
//// 11.28 新增：
//void TaskCSysDragger::onDistanceEditingFinished()
//{
//    if (!m_vpDragger || !distanceSpinBox) {
//        return;
//    }
//
//    // QuantitySpinBox 的 rawValue 通常就是内部基准单位（此处是 mm）
//    double newDistance = distanceSpinBox->rawValue();
//
//    m_vpDragger->applyNumericTranslation(newDistance);
//}
